...
{
    public partial class JediniceMjereControl : UserControl
    {
        private List<JedinicaMjere> _jedinice = new List<JedinicaMjere>();
        private JedinicaMjere _odabrana = null;

        private Guna2DataGridView dgvJedinice;
        ...
        private void KreirajHeader()
        {
            ...
            pnl.Controls.Add(new Label { Text = "📐  Jedinice mjere", Font = AppFonts.TitleLarge, ForeColor = AppColors.TextPrimary, Location = new Point(0, 2), AutoSize = true });
        ...
        private void KreirajToolbar()
        {
            ...
            int y = 10;
            txtPretraga = new Guna2TextBox { PlaceholderText = "🔍  Pretraži jedinice mjere...", Size = new Size(220, 36), Location = new Point(10, y), FillColor = AppColors.Background, BorderColor = AppColors.BorderLight, Font = AppFonts.Regular, ForeColor = AppColors.TextPrimary, BorderRadius = 8 };
            ...
            txtPretraga.TextChanged += (s, e) => UcitajJedinice();
            pnl.Controls.Add(txtPretraga);

            btnOsvjezi = KreirajGumb("🔄  Osvježi", AppColors.Secondary); btnOsvjezi.Click += (s, e) => UcitajJedinice(); pnl.Controls.Add(btnOsvjezi);
        ...
        private void KreirajGrid()
        {
            ...
            dgvJedinice.DefaultCellStyle.BackColor = AppColors.CardBackground;
            dgvJedinice.DefaultCellStyle.ForeColor = AppColors.TextPrimary;
            dgvJedinice.DefaultCellStyle.SelectionBackColor = AppColors.Primary;
            dgvJedinice.DefaultCellStyle.SelectionForeColor = Color.White;
            dgvJedinice.DefaultCellStyle.Font = AppFonts.Regular;
            dgvJedinice.DefaultCellStyle.Padding = new Padding(4, 0, 4, 0);
            dgvJedinice.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(248, 249, 252);
            dgvJedinice.ColumnHeadersDefaultCellStyle.BackColor = AppColors.Secondary;
            dgvJedinice.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            dgvJedinice.ColumnHeadersDefaultCellStyle.Font = AppFonts.RegularMedium;

            foreach (var k in new[] {
                (Name: "colId",          Header: "ID",          Min: 50,  Weight: 50f,  Align: DataGridViewContentAlignment.MiddleCenter),
                (Name: "colNaziv",       Header: "Naziv",       Min: 200, Weight: 300f, Align: DataGridViewContentAlignment.MiddleLeft),
                (Name: "colSkracenica",  Header: "Skraćenica",  Min: 100, Weight: 120f, Align: DataGridViewContentAlignment.MiddleCenter),
            })
            {
                ...
                dgvJedinice.Columns.Add(col);
            }

            dgvJedinice.SelectionChanged += (s, e) => {
                _odabrana = dgvJedinice.SelectedRows.Count > 0 && dgvJedinice.SelectedRows[0].Index < _jedinice.Count ? _jedinice[dgvJedinice.SelectedRows[0].Index] : null;
                btnUredi.Enabled = _odabrana != null; btnObrisi.Enabled = _odabrana != null;
            };
            dgvJedinice.CellDoubleClick += (s, e) => { if (e.RowIndex >= 0) BtnUredi_Click(s, e); };

            pnlGrid.Controls.Add(dgvJedinice);
            pnlGrid.Controls.Add(lblStatus);
            this.Controls.Add(pnlGrid);
        }

        private void UcitajJedinice()
        {
            try
            {
                this.Cursor = Cursors.WaitCursor;
                _jedinice = JediniceMjereRepository.GetSveJedinice(txtPretraga?.Text ?? "");
                dgvJedinice.Rows.Clear();
                foreach (var j in _jedinice)
                    dgvJedinice.Rows.Add(j.Id, j.Naziv, j.Skracenica);
                _odabrana = null;
                btnUredi.Enabled = false; btnObrisi.Enabled = false;
                lblBroj.Text = $"Ukupno {_jedinice.Count} jedinica mjere";
                lblStatus.Text = $"  Prikazano: {_jedinice.Count} rezultata";
            }
        ...
        { if (new frmJedinicaMjere().ShowDialog() == DialogResult.OK) UcitajJedinice(); }

        private void BtnUredi_Click(object sender, EventArgs e)
        {
            if (_odabrana == null) return;
            var j = JediniceMjereRepository.GetJedinicaById(_odabrana.Id);
            if (j != null && new frmJedinicaMjere(j).ShowDialog() == DialogResult.OK) UcitajJedinice();
        }

        private void BtnObrisi_Click(object sender, EventArgs e)
        {
            if (_odabrana == null) return;
            if (MessageBox.Show($"Sigurno želiš obrisati jedinicu mjere:\n\n{_odabrana.Naziv} ({_odabrana.Skracenica})?", "Potvrda", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes) return;
            try { JediniceMjereRepository.Obrisi(_odabrana.Id); UcitajJedinice(); }
